...
category == "Median" ~ 6,
category == "Std Dev" ~ 7,
category == "Range(min,max)" ~ 8
))
 
dm_allcomb1 <- dm_allcomb[order(dm_allcomb$sortn), ]
dm_allcomb1
```
...
```{r}
dm_allcomb2 <- bind_rows(dm_allcomb1[4,], dm_allcomb1[1:3,], dm_allcomb1[5:8,])  
dm_allcomb2
```
...
ethnic_data_new <- ethnic_mrg_cnt3 %>% 
  mutate(perc_eth_tot = round((ethnic_total/N)*100, 1)) %>%                 
  mutate(perc_eth = round((ethnic_n/n)*100,1))

ethnic_pct <- ethnic_data_new %>%
  mutate(perc_e_tchar = format(perc_eth_tot, nsmall = 1)) %>%
  mutate(perc_e_char = format(perc_eth, nsmall = 1))

ethnic_n_pct <- ethnic_pct %>%
  mutate(npct = paste(ethnic_n, paste0("(", perc_e_char, ")"))) %>% 
...
race_data_new <- race_mrg_cnt3 %>% 
  mutate(perc_eth_tot = round((race_total/N)*100, 1)) %>%                 
  mutate(perc_eth = round((race_n/n)*100,1))
...
race_n_pct <- race_pct %>%
  mutate(npct = paste(race_n, paste0("(", perc_e_char, ")"))) %>% 
...
dm_allcomb3 <- bind_rows(dm_allcomb2, ethnic_cat, race_cat)  
dm_allcomb3
```
