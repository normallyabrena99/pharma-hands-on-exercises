...
```{r}
plot2 <- plot1 + 
  labs(title = "Individual Heart Rate Plot of Participants",
       subtitle = "Domain: Vital Signs",
       x = "Observation Day",
...
```{r}
VS <- import("https://github.com/phuse-org/phuse-scripts/raw/master/data/sdtm/updated_cdiscpilot/vs.xpt")

vs_pulse <- VS %>% 
    filter(VSTESTCD== "PULSE") %>% 
    filter(VISITNUM > 3)

pulse_plot1 <- ggplot(data = (vs_pulse), mapping = aes(x=VSDY, y=VSSTRESN, group =  USUBJID)) +
  geom_line()

pulse_plot2 <- pulse_plot1 + 
  labs(title = "Individual Heart Rate Plot of Participants",
       subtitle = "Domain: Vital Signs",
       x = "Observation Day",
       y = "Pulse Rate (BEATS/MIN)",
