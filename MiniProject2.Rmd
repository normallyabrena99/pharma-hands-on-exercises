...
-   filter(.data = adsl, EFFFL == "Y") 
#this second one since we must have double equal sign for character “Y”
-   filter(.data = adsl, "EFFFL" == "Y")
-   filter(.data = adsl, EFFFL = "y")
...
paste(n, "(",perc_char,")") # 6 ( 26.1 )
```

```{r}
paste(n, paste0("(",perc_char,")")) # 6 (26.1)
```

```{r}
paste(n, "(",perc_char,")", sep="") # 6(26.1)
```

```{r}
paste0(n, "(",perc_char,")") # 6(26.1)
```
...
```{r challenge2}
# Paste commands above after the "<-" assignment, one per line
adae_aebodsys_cnt <- small_n_adae %>%
                left_join(BigN_adae) %>%
                mutate(perc = round(n/N*100, digits=1)) %>%
                mutate(perc_char = format(perc, nsmall=1)) %>%
                mutate(npct = paste(n, "(", perc_char, ")" ) ) %>% 
                mutate(AEBODSYS = str_to_title(AEBODSYS)) %>% 
                ungroup() %>%
